...
**Live proof (M3x, 2026-07-10):** OAuth refreshed (`claude auth status` →
logged in). `claude -p "say ok"` OK when `ANTHROPIC_BASE_URL` is unset
(user `.zshrc` points it at a dead local proxy — spawn for this proof used
`unset ANTHROPIC_BASE_URL` in the tmux shell cmd; same `-L hypervisor`
socket + `approvals::{approve,deny}` send-keys as Tab / deny).

**Live pane capture (pre-approve):**
```
 Bash command
...
**Parser diffs vs prior fixture:** live TUI no longer prints `Bash(cmd)` as
a single tool line; it uses a `Bash command` block + argv line. Option 3
is now `No` (not `No, and tell Claude what to do differently`).
`parse_claude_pane` updated → `Bash(./scripts_build.sh)`; legacy fixture
still parses. Unit tests green (legacy + live).

**Approve proof:** `send-keys 1 Enter` → dialog cleared; pane shows
`hello-from-m3x` from `./scripts_build.sh`.

**Deny-with-guidance proof:** `send-keys 3 Enter` → tool_result
`The user doesn't want to proceed… rejected`; then guidance
`do not run the script — reply with the word DENIED_M3X instead` arrived
as a user turn; assistant replied `DENIED_M3X` (transcript
`49ad00c6-…jsonl`).

**Deny timing:** after `3 Enter`, input prompt returned in **~59ms**.
The 400ms sleep in `approvals::deny` is enough — no race; left as-is.

**send-keys:** approve = `1` then `Enter`; deny = `3` then `Enter`, then
guidance via normal `tmux::send`.
### Codex
...
**CLOSED in M3x (2026-07-10).** Live pane capture, approve (`1`→`hello-from-m3x`),
and deny-with-guidance (`3`→guidance→`DENIED_M3X`) recorded above; parser
re-derived from the live dialog.
